...
Choose a model from Hugging Face (or search AWS JumpStart directly in-tool), and Magemaker will spin up an instance with a ready-to-query endpoint of the model in minutes.
...
* Python 3.11+ (3.12 is not supported; 3.13 is not yet supported due to an open Azure SDK issue)
* Cloud Configuration
...
* Certain Hugging Face models (e.g. Llama 2 / Llama 3) require an access token ([hf docs](https://huggingface.co/docs/hub/en/models-gated#access-gated-models-as-a-user))
...
* Search and deploy Hugging Face or **AWS JumpStart** models
...
```yaml
training: !Training
  destination: aws  # or gcp, azure (currently AWS-only)
...
- **Type:** Fill Mask – tries to complete your sentence like Mad Libs
...
- **Type:** Feature extraction – turns text into a 384-dim embedding for semantic search / clustering
...
Any model endpoints you spin up will run continuously unless you deactivate them! Make sure to delete endpoints you’re no longer using so you don’t keep getting charged for your SageMaker (or other cloud) instances.
