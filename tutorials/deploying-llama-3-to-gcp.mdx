...
deployment: !Deployment
  ...
  accelerator_type: NVIDIA_TESLA_T4  # Updated to match Vertex AI enum
  num_gpus: 1
  quantization: null
...
```
<Note>
   For gated models like Llama from Meta, you must accept the terms of use on Hugging Face and add your Hugging Face token to the environment for deployment to succeed.
</Note>


### Selecting an Appropriate Instance
For Llama 3, a machine type such as `n1-standard-8` with an attached NVIDIA T4 GPU (`NVIDIA_TESLA_T4`) is a suitable configuration for most use cases. Adjust the instance type and GPU based on your workload requirements.
...
def query_vertexai_endpoint_rest(
    ...
    # Project configuration from your .env file
