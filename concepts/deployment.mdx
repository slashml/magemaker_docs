...
> **Tip**  
> All examples below use the `!Deployment` and `!Model` tags that Magemaker registers with PyYAML. If you copy an example make sure those tags are present.
...
deployment: !Deployment
  ...
  instance_count: 1
  instance_type: n1-standard-4  # previously `machine_type`
...
models:
  - !Model
    id: facebook--opt-125m  # note the double hyphen in Azure IDs
    source: huggingface
```
...
deployment: !Deployment
  ...
  num_gpus: 4          # optional, only for GPU instances
  quantization: null   # optional: bitsandbytes | awq | null
...
<Note>
Both `num_gpus` and `quantization` are optional. Use them when deploying very large models that need multi-GPU inference or quantised weights.
</Note>
...
<CardGroup>
  ...
  <Card title="ml.g5.12xlarge" icon="server">
    Required for larger models like LLaMA-3-8B  
    • 48 vCPU  
    • 192 GB Memory  
    • 4 NVIDIA A10G GPUs
  </Card>
...
3. **Model Loading Failure**  
   ...
   • Endpoint deployed but model failed to load—check logs and report the issue if it persists.
